...
describe("generateStepHandlers", () => {
  test("should generate handler for a 'begin' step", async () => {
    const flowJson = {
    ...
    const result = await generateStepHandlers(flowJson, 1);
    expect(result.replace(/\s/g, "")).toBe(expected.replace(/\s/g, ""));
  });

  test("should generate handler for an 'end' step", async () => {
    const flowJson = {
  ...
  test("should generate handler for a 'task' step", async () => {
    const flowJson = {
  ...
  test("should generate handler for a 'decision' step with conditions", async () => {
    const flowJson = {
  ...
  test("should handle different baseIndentLevel", async () => {
    const flowJson = {
      steps: [{ id: "begin", type: "begin" }],
      connections: [{ sourceStepId: "begin", targetStepId: "firstTask" }],
    } as Flow;
    ...
    const result = await generateStepHandlers(flowJson, 2);
    expect(result.replace(/\s/g, "")).toBe(expected.replace(/\s/g, ""));
  });
});
