...
export const parseEnum = (
  jsonSchema: JsonSchemaObject & { enum: Serializable[] }
) => {
  ...
  if (jsonSchema.enum.length === 1) {
    ...
    return z.literal(jsonSchema.enum[0] as z.util.Literal);
  }
  ...
  return z.union(
    jsonSchema.enum.map((x) => z.literal(x as z.util.Literal)) as unknown as [
      z.ZodTypeAny,
      z.ZodTypeAny
    ]
  );
